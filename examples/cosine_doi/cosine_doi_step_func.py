...
# Define the step function to be registered for SIRBreed (in cosine_doi_breed.py)
...
def step_func(
    ...
    infection_history : cupy.array
        1D cupy array of infection history for each agent.
    Returns:
    -------
    None
    ...
    if agent_state == 2 and rand < p_recovery:
        ...
        while i < len(neighbor_ids) and not cp.isnan(neighbor_ids[i]):
            ...
            if neighbor_state == 2 and rand < p_infection * (
                ...
                agent_state = 2  # Agent becomes infected
                set_this_agent_data_from_tensor(
                    agent_index, state_tensor, agent_state
                )  # Agent becomes infected
            i += 1

    # Update the infection history for the agent
    tick = int(globals[0])
    infection_history[agent_index][tick] = int(agent_state)
