...
do
    ...
    do
        ...
        do
            num_init_connections=$( printf "%.0f" ${percent_init_connections}*${num_agents} )
            num_mpi_ranks=8*${num_nodes}
            # Run script
            echo Running Python Script
            time srun -N${num_nodes} -n${num_mpi_ranks} -c7 --gpus-per-task=1 --gpu-bind=closest python3 -u ${SRC_DIR}/run.py ${num_agents} ${num_init_connections} ${num_nodes}
            echo Run Finished
            date
        done
    done
